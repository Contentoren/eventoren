...
import { userRoleToCurrent } from "#src/auth/model_field/userRole.ts"
...
export function docUserToUserProfile(u: DocUser, orgHandle?: string, orgRole?: OrgRole): UserProfile {
  const {
    ...
    zitadelUserId,
    zitadelRoles,
    zitadelRolesSynchronizedAt,
    organizerInvitedAt,
    role,
    // rest
    ...rest
  } = u
  const profile: UserProfile = { userId: _id as string, ...rest, role: userRoleToCurrent(role) }
