...
export function UiStepper(props: { labels: readonly string[]; currentIndex: number; class?: string }) {
  ...
  return (
    <ol class={`flex flex-wrap items-center gap-space-4 ${props.class ?? ""}`} aria-label={state.progressLabel()}>
      <For each={state.items()}>
        {(item) => (
          <li class="flex items-center gap-space-3" aria-current={item.status === "current" ? "step" : undefined}>
            ...
            <span class={`text-sm font-medium ${item.status === "upcoming" ? "text-content-muted" : "text-content"}`}>
              {item.label}
            </span>
          </li>
        )}
