...
Billing's worker starts with the Billing application bootstrap, performs an initial bounded recovery, then wakes on new work and its interval. It must prepare one combined ticket PDF, one invoice PDF, and the two accepted legal PDFs; it sends exactly one email only after all four artifacts exist. Live mode obtains the invoice PDF from Lexware. Test mode renders a clearly marked non-fiscal invoice artifact from the persisted purchase snapshot without Lexware calls; it does not create a fiscal invoice or infer a missing tax policy. A `sending` or `unknown` confirmation state is reported as `manual_reconciliation_required`, not as a generic retryable `failed` result.
